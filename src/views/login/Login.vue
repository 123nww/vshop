...
export default {
  data() {
    return {
      ...
      //imageURL:  require('@/assets/img/login/loginpage.svg'),
      imageURL: "",
  ...
  methods: {
    // 0.mapActions 同步用户信息
    ...mapActions(["syncuserInfo", "ISSHOWMAINTAR"]),
    // 1.账号密码登录及短信验证码切换
    switchLogin() {
    ...
    changeImage(index) {
      if (index == 0) {
        this.imageURL = require("@/assets/img/login/greeting.png");
      } else if (index == 1) {
        this.imageURL = require("@/assets/img/login/blindfold.png");
      } else {
        this.imageURL = require("@/assets/img/login/normal.png");
      }
    },
    // 3.账号密码登录切换图片验证码
    getCaptcha() {
